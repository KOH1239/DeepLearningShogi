...
class PolicyValueABNetwork(nn.Module):
    def __init__(self):
        super(PolicyValueABNetwork, self).__init__()
        ...
        self.att_l2_v = nn.Conv2d(in_channels=MAX_MOVE_LABEL_NUM, out_channels=1, kernel_size=1, bias=False)
        self.att_norm2_v = nn.BatchNorm2d(1)
    ...
    def forward(self, x1, x2):
        ...
        # policy attention branch
        att1_p = self.swish(self.att_norm1_p(self.att_l1_p(h21)))
        ...
        # policy network
        h22 = self.l22(u21 * self.att_p + u21)
        h22_1 = self.l22_2(h22.view(-1, 9*9*MAX_MOVE_LABEL_NUM))
        # value attention branch
        att1_v = self.swish(self.att_norm1_v(self.att_l1_v(h21)))
        ...
        # value network
        h22_v = self.swish(self.norm22_v(self.l22_v(u21 * self.att_v + u21)))
